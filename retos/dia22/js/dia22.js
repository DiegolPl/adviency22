...
let btnPrevisualizar = document.getElementById('btn_previsualizar');
let btnMusicaOff = document.getElementById('btn_musica_off');
let btnMusicaOn = document.getElementById('btn_musica_on');
let musica = document.getElementById('musica');

//Control de la musica
btnMusicaOff.addEventListener('click',()=>{
    btnMusicaOff.classList.toggle('parlanteActivo');
    btnMusicaOn.classList.toggle('parlanteActivo');
    musica.removeAttribute('muted')
    musica.muted = false
    musica.volume = 0.08203125
})

btnMusicaOn.addEventListener('click',()=>{
    btnMusicaOff.classList.toggle('parlanteActivo');
    btnMusicaOn.classList.toggle('parlanteActivo');
    musica.setAttribute('muted','true');
    musica.muted = true
})
...
function generarLista(regalos){
    ...
    if(gifts.length > 0){
        ...
        regalos.forEach(regalo => {
            ...
            total += parseFloat(regalo.precioTotal);
            ...
            let duplicar = document.createElement('BUTTON');
            duplicar.classList.add('material-symbols-outlined');
            duplicar.classList.add('btn_duplicar');
            duplicar.setAttribute('title','Duplicar');
            duplicar.setAttribute('tabindex','2');
            duplicar.innerText = 'content_copy';

            duplicar.addEventListener('click',()=>{
                btnOpenForm.click();
                document.getElementById('gift_input').focus();  
                document.getElementById('gift_input').value = regalo.nombre;
                document.getElementById('gift_link').value = regalo.link;
                document.getElementById('gift_cantidad').value = regalo.cantidad;
                document.getElementById('gift_price').value = regalo.precio;
                document.getElementById('gift_owner').value = regalo.owner;
            })

            contenedorAcciones.appendChild(duplicar);
            ...
            edit.addEventListener('click',()=>{
                ...
                document.getElementById('gift_input_edit').focus();
            ...
            tachito.setAttribute('tabindex','2')
            ...
            tachito.addEventListener('click',()=>{
                Swal.fire({
                    ...
                    if (result.isConfirmed) {
                        ...
                        let parcial = 0
                        gifts.forEach(regalo => {
                            parcial += parseFloat(regalo.precioTotal);
                        })
                        document.getElementById('valorTotal').innerText = parcial.toFixed(2);
...
btnPrevisualizar.addEventListener('click',()=>{
    ...
    contenedor.setAttribute('id','contenedor_lista_prev');
    contenedor.classList.add('contenedor_lista_prev');
    contenedor.classList.add('d-grid');
    document.getElementById('box_app').appendChild(contenedor);

    let lista = document.createElement('DIV');
    lista.classList.add('lista_previsualizacion');
    contenedor.appendChild(lista);

    let titulo = document.createElement('H2');
    titulo.innerText = 'Lista de Regalos';
    titulo.classList.add('titulo_form');
    ...
    btnClose.classList.add('close_form_gift');
    btnClose.setAttribute('id','close_form_edit_gift');
    btnClose.setAttribute('type','button');
    btnClose.setAttribute('tabindex','2');
    ...
    let botonPrint = document.createElement('BUTTON');
    botonPrint.classList.add('btn_print');
    botonPrint.innerText = 'Imprimir';
    botonPrint.addEventListener('click',()=>{
       print();
    })
    lista.appendChild(botonPrint);
})
